...
loadLottieAnimation(".lottie-1", "assets/json/winner.json");
loadLottieAnimation(".lottie-2", "assets/json/lottie-2.json");
loadLottieAnimation(".lottie-3", "assets/json/lottie-3.json");
...
function updatePodiumHeight(totalVitorias) {
    if (totalVitorias > 0) {
...
function generateScoreboardEntry(jogador, totalVitorias, marcador) {
    return `
        <div class="score">
            <h4 class="col-lg-2 marker-ctn">
                <img src="${marcador}" alt="marker" class="marker">
                ${totalVitorias > 0 ? jogador.posicao + "º" : "-"}
            </h4>
            <div class="col-lg-4 ctn-img">
                <img class="avatar" src="${jogador.avatar}" alt="avatar" /> ${jogador.nome}
            </div>
...
function generatePodiumEntry(jogador, totalVitorias, marcador, posicao) {
    return `
        <div class="col-lg-3" id="podium">
        <div id="lottie">
            <div class="festa"></div>
            <div class="lottie-${posicao} lottie"></div>
            <div class="avatar">
    ...
    <div class="bar col-lg-3">
        <span class="number">${posicao}º</span>
        <img src="${marcador}" alt="marker" class="marker">
        <div class="fire-${posicao}"></div>
    </div>
    `;
}

function updateUI(jogadores) {
    jogadores.forEach((jogador, index) => {
        const marcadores = jogador.marcadores || {};
        const datas = Object.keys(marcadores);
        const totalVitorias = Object.values(jogador.vitorias).reduce((acc, cur) => acc + cur, 0);
        const marcador = marcadores[datas[datas.length - 1]] || neutralMarker;

        updatePodiumHeight(totalVitorias);

        jogador.posicao = index + 1;

        if (index < 3) {
            podium.innerHTML += generatePodiumEntry(jogador, totalVitorias, marcador, jogador.posicao);
        } else {
            scoreBoard.innerHTML += generateScoreboardEntry(jogador, totalVitorias, marcador);
        }
    });
}

function sortPlayersByTotalWins(jogadores) {
...
function updatePositions(jogadores) {
    Object.keys(jogadores[0].vitorias)
        ...
        .forEach((data, index, datas) => {
            ...
            const rankingAtual = generateRanking(jogadores, data);
            const rankingAnterior = index > 0 ? generateRanking(jogadores, datas[index - 1]) : null;
...
function generateRanking(jogadores, data) {
    return jogadores.slice().sort((a, b) => {
        const totalVitoriasA = calculateTotalWinsUntilDate(a, data);
        const totalVitoriasB = calculateTotalWinsUntilDate(b, data);
        return totalVitoriasB - totalVitoriasA;
    });
}

function calculateTotalWinsUntilDate(jogador, data) {
    let totalVitorias = 0;
...
loadPlayers((err, jogadores) => {
    if (err) {
        console.error("Erro ao ler os dados dos jogadores:", err);
    } else {
        jogadores = sortPlayersByTotalWins(jogadores);
        updatePositions(jogadores);
        updateUI(jogadores);
    }
});
